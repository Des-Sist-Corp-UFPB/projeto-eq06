...
class ProdutoServiceTest {
    ...
    void setUp() {
        ...
        formValido = new ProdutoForm("Feijão Preto", "Feijão preto premium", new BigDecimal("7.50"), null);
    }

    // =========================================================================
    // TESTES: listar e buscar
    // =========================================================================

    @Test
    @DisplayName("listar: deve retornar pagina de produtos")
    void listar_deveRetornarPaginaDeProdutos() {
        org.springframework.data.domain.Page<Produto> page = new org.springframework.data.domain.PageImpl<>(java.util.List.of(produtoExistente));
        org.springframework.data.domain.PageRequest pageRequest = org.springframework.data.domain.PageRequest.of(0, 10);
        when(produtoRepository.findAll(pageRequest)).thenReturn(page);

        org.springframework.data.domain.Page<Produto> resultado = produtoService.listar(pageRequest);
    ...
    @Test
    @DisplayName("buscar: com string vazia deve retornar findAll")
    void buscar_comStringVazia_deveRetornarFindAll() {
        org.springframework.data.domain.Page<Produto> page = new org.springframework.data.domain.PageImpl<>(java.util.List.of(produtoExistente));
        org.springframework.data.domain.PageRequest pageRequest = org.springframework.data.domain.PageRequest.of(0, 10);
        when(produtoRepository.findAll(pageRequest)).thenReturn(page);

        org.springframework.data.domain.Page<Produto> resultado = produtoService.buscar("   ", pageRequest);

        assertThat(resultado).isNotNull();
        verify(produtoRepository, times(1)).findAll(pageRequest);
        verify(produtoRepository, never()).findByNomeContainingIgnoreCase(anyString(), any());
    }

    @Test
    @DisplayName("buscar: com string valida deve retornar findByNomeContainingIgnoreCase")
    void buscar_comStringValida_deveRetornarFindByNome() {
        org.springframework.data.domain.Page<Produto> page = new org.springframework.data.domain.PageImpl<>(java.util.List.of(produtoExistente));
        org.springframework.data.domain.PageRequest pageRequest = org.springframework.data.domain.PageRequest.of(0, 10);
        when(produtoRepository.findByNomeContainingIgnoreCase("Arroz", pageRequest)).thenReturn(page);

        org.springframework.data.domain.Page<Produto> resultado = produtoService.buscar(" Arroz ", pageRequest);

        assertThat(resultado).isNotNull();
        verify(produtoRepository, times(1)).findByNomeContainingIgnoreCase("Arroz", pageRequest);
        verify(produtoRepository, never()).findAll(any(org.springframework.data.domain.Pageable.class));
    }
